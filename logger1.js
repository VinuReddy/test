...
LOGGER.send = function(level, data) {
    if (!LOGGER.socket) {
        LOGGER.socket = angular.element(document.body).injector().get('$socket');
    }
    LOGGER.socket.send(JSON.stringify({
        type: 'LOG',
        options: {
            message: data,
            logLevel: level
...
LOGGER.print = function(level, message, loggerName) {
    ...
    logMessage += ' ' + message;
    if (!LOGGER.CONFIG) {
        LOGGER.CONFIG = angular.element(document.body).injector().get('CONFIG');
    }
    if (LOGGER.CONFIG) {
    ...
    if (LOGGER.settings.enable && LOGGER.determineLevelNumber(level) >= LOGGER.determineLevelNumber(LOGGER.settings.level)) {
